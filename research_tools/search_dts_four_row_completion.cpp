...
constexpr int kRowsToComplete = 4;
...
struct PackingSearch {
    ...
    void search(const Mask& used, int depth, int gaps, const Mask& gap_mask) {
        ...
        for (int index : by_difference[anchor]) {
            ...
            const Mask next_used{used.low | catalog[index].mask.low, used.high | catalog[index].mask.high};
            search(next_used, depth + 1, gaps, gap_mask);
            if (found || timed_out) return;
        }
        if (gaps < 6) {
...
int main(int argc, char** argv) {
    if (argc != 3) {
        std::cerr << "usage: search_dts_four_row_completion SECONDS OUTPUT\n";
        return 2;
    }
    ...
    if (!output) return 3;
    output << "{\n"
           << "  \"method\":\"exact-four-row-packing-against-three-compatible-rows\",\n"
           << "  \"scope_limit\":111,\n"
           << "  \"fixed_row_count\":3,\n"
