...
## Phase 2 evidence after forty-three substantive experiments
...
## Phase 2 evidence after sixty-eight substantive experiments
...
## DTS exact row-universe capability audit (no Experiment 74)

- The complete normalized valid six-mark ruler universe at scope 111 was enumerated exactly: `78,787,474` valid rows from `128,164,707` raw choices. The local binary catalogue is `1,890,899,400` bytes with SHA-256 `07088897F6640184B5B12516AF71E9C0CE7B3E362C78EA91265DF19A72D2E52E`.
- Native enumeration matched an independent Python combinations reference at scopes `25`, `35`, and `50`. A deterministic sample of `32` full-catalogue rows passed repository-verifier row-level difference arithmetic, and the canonical scope-112 seven-row baseline passed the full repository verifier.
- The catalogue is a tractable reusable substrate. Controlled depth-two scans found `516,086` to `6,335,467` compatible second rows; sampled depth-three scans found `2,041` to `17,361` compatible third rows; two sampled depth-four probes found zero compatible fourth rows. A difference-column bitset index used `1.09 GB` and did not remove the depth-seven combinatorial bottleneck.
- Decision: `INTRACTABLE_CURRENT_CAPABILITY` for complete scope-111 seven-row packing with the measured representations and host. This is not a proof of DTS infeasibility, no scientific search was launched, `experiments_run` remains `73`, and Experiment 74 was not created.
- Evidence: `artifacts/dts-capability-audit.json`, `artifacts/dts-ruler-catalogue-manifest.json`, `artifacts/dts-packing-benchmark.json`, `artifacts/dts-index-benchmark.json`, and `artifacts/dts-ruler-catalogue-sample-audit.json`.
